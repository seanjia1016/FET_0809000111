...
public class CreateTxn {
	...
	public void function(SCESession mySession, JSONObject ResponseJsonObject) {
		...
		String testMode = complexVariable.getField(IProjectVariables.PROPERTIES_FIELD_TEST__MODE).getStringValue();
		
		mySession.getTraceOutput().writeln(ITraceInfo.TRACE_LEVEL_INFO,"CreateTx_Output ResponseJsonObject Response:" + ResponseJsonObject);
			...
			if (ResponseJsonObject instanceof JSONObject) {
				...
				if (!ResponseJsonObject.has("Data")) {
					...
					mySession.getTraceOutput().writeln(ITraceInfo.TRACE_LEVEL_INFO,"Exception Response:" + Exception);
					...
					if (fragment.has("ns0:txnId")) {
						...
						IVariableField txnId_varible = mySession.getVariableField(
										IProjectVariables.CREATE_TXN__OUTPUT,
										IProjectVariables.CREATE_TXN__OUTPUT_FIELD_TXN_ID);
						txnId_varible.setValue(txnId);
						
						mySession.getTraceOutput().writeln(ITraceInfo.TRACE_LEVEL_INFO,"##CreateTx_CIE_Output TxnId Response:" + txnId);	
					}
	...
	public void assignJsonval(SCESession mySession, JSONObject returnHeaderval) {
		...
		mySession.getTraceOutput().writeln(ITraceInfo.TRACE_LEVEL_INFO,"##CreateTx_CIE_Output returnCodeval Response:" + returnCodeval);
...
//				"legacyCodeval Response:" + legacyCodeval);
		...
		mySession.getTraceOutput().writeln(ITraceInfo.TRACE_LEVEL_INFO,"##CreateTx_CIE_Output processIDval Response:" + processIDval);
		...
		mySession.getTraceOutput().writeln(ITraceInfo.TRACE_LEVEL_INFO,"##CreateTx_CIE_Output processNameval Response:" + processNameval);
		...
		mySession.getTraceOutput().writeln(ITraceInfo.TRACE_LEVEL_INFO,"##CreateTx_CIE_Output returnMesgval Response:" + returnMesgval);
